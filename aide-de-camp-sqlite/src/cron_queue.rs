...
// Helper to convert SQLite INTEGER (Unix timestamp) to DateTime
fn timestamp_to_datetime(ts: i64) -> DateTime {
    use sqlx::types::chrono::TimeZone;
    Utc.timestamp_opt(ts, 0).unwrap()
}

// Helper to convert DateTime to SQLite INTEGER (Unix timestamp)
...
impl CronQueue for SqliteQueue {
        ...
        J::Payload: Serialize,
    {
        ...
        // Serialize payload
        let payload_bytes =
            serde_json::to_vec(&payload).map_err(|e| CronError::ParseError(e.to_string()))?;
